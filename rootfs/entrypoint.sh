...
# Originally copied from https://github.com/jnovack/autossh and adjusted

source version.sh

if [ -z "${SSH_PRIVATE_KEY}" ]; then
    echo "[FATAL ] No SSH private key provided"
    exit 1
fi
eval $(ssh-agent -s)
SSH_PRIVATE_KEY_MULTILINE=$( echo "${SSH_PRIVATE_KEY}" | sed 's/-----BEGIN OPENSSH PRIVATE KEY----- //' | sed 's/ -----END OPENSSH PRIVATE KEY-----//' | tr ' ' '\n')
echo -e "-----BEGIN OPENSSH PRIVATE KEY-----\n${SSH_PRIVATE_KEY_MULTILINE}\n-----END OPENSSH PRIVATE KEY-----" | ssh-add -k -
